...
CREATE TABLE `USER` (
    ...
    INDEX `USER_USER_UUID_idx`(`USER_UUID`),
    PRIMARY KEY (`USER_NO`)
...
CREATE TABLE `USER_OAUTH` (
    ...
    `USER_OAUTH_TOKEN` VARCHAR(191) NOT NULL,

    INDEX `USER_OAUTH_USER_NO_idx`(`USER_NO`),
    INDEX `USER_OAUTH_USER_OAUTH_PROVIDER_USER_OAUTH_TOKEN_idx`(`USER_OAUTH_PROVIDER`, `USER_OAUTH_TOKEN`),
    UNIQUE INDEX `USER_OAUTH_USER_OAUTH_PROVIDER_USER_NO_key`(`USER_OAUTH_PROVIDER`, `USER_NO`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `PROBLEM_CONTENT` (
    `PROBLEM_CONTENT` VARCHAR(191) NOT NULL,
...
CREATE TABLE `PROBLEM_INPUT_OUTPUT` (
    `PROBLEM_INPUT_OUTPUT_NO` INTEGER NOT NULL AUTO_INCREMENT,
    `PROBLEM_INPUT` VARCHAR(191) NOT NULL,
    `PROBLEM_OUTPUT` VARCHAR(191) NOT NULL,
    `PROBLEM_NO` INTEGER NOT NULL,

    INDEX `PROBLEM_INPUT_OUTPUT_PROBLEM_NO_idx`(`PROBLEM_NO`),
    UNIQUE INDEX `PROBLEM_INPUT_OUTPUT_PROBLEM_INPUT_OUTPUT_NO_PROBLEM_NO_key`(`PROBLEM_INPUT_OUTPUT_NO`, `PROBLEM_NO`),
    PRIMARY KEY (`PROBLEM_INPUT_OUTPUT_NO`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
