...
export class ProblemsCollectService {
    ...
    async collect({url, userNo}:{url: string, userNo: number}) {
        const requestCount = await this.redisService.get(`problemCollectCount_${userNo}`);
        ...
        const contentList = await this.postProcess(result.data.contentList, site, sourceId);
    ...
    private parse(url: string) {
        if (url.includes('https://www.acmicpc.net/problem/')) {
            const sourceId = url.split('problem/')[1]
                .split('/')[0]
                .trim();
